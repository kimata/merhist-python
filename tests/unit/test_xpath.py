...
class TestNthElement:
    ...
    def test_first_element(self):
        """1番目の要素"""
        result = merhist.xpath.nth_element("//div", 1)
        assert result == "(//div)[1]"

    def test_tenth_element(self):
        """10番目の要素"""
        result = merhist.xpath.nth_element("//ul/li", 10)
        assert result == "(//ul/li)[10]"

    def test_complex_xpath(self):
        """複雑なXPath"""
        base = '//div[@class="container"]//span'
        result = merhist.xpath.nth_element(base, 5)
...
class TestSoldItemColumn:
    ...
    def test_first_column(self):
        """1番目のカラム"""
        result = merhist.xpath.sold_item_column("//tr[1]", 1)
        assert result == "(//tr[1]//td)[1]"

    def test_ninth_column(self):
        """9番目のカラム"""
        result = merhist.xpath.sold_item_column("//tr[5]", 9)
...
class TestSelectorConstants:
    ...
    def test_navigation_top_exists(self):
        ...
        assert merhist.xpath.NAVIGATION_TOP
        assert "merNavigationTop" in merhist.xpath.NAVIGATION_TOP

    def test_sold_list_item_exists(self):
        """SOLD_LIST_ITEM が定義されている"""
        assert merhist.xpath.SOLD_LIST_ITEM
        assert "listing-container" in merhist.xpath.SOLD_LIST_ITEM

    def test_bought_list_exists(self):
        """BOUGHT_LIST が定義されている"""
        assert merhist.xpath.BOUGHT_LIST
        assert "purchase-item-list" in merhist.xpath.BOUGHT_LIST
