...
# === フィクスチャ ===
@pytest.fixture
def db(tmp_path: pathlib.Path) -> Generator[merhist.database.Database, None, None]:
    """テスト用データベースフィクスチャ"""
    db_path = tmp_path / "test.db"
    database = merhist.database.Database(db_path, SCHEMA_PATH)
    yield database
    database.close()


@pytest.fixture
def sample_sold_item() -> merhist.item.SoldItem:
...
@pytest.fixture
def sample_bought_item() -> merhist.item.BoughtItem:
...
class TestDatabaseInit:
    ...
    def test_creates_tables(self, db: merhist.database.Database):
        """テーブルが作成される"""
        conn = db._get_conn()
...
class TestSoldItemCRUD:
    ...
    def test_upsert_new_item(self, db: merhist.database.Database, sample_sold_item: merhist.item.SoldItem):
        """新規アイテムの挿入"""
        db.upsert_sold_item(sample_sold_item)
        assert db.exists_sold_item(sample_sold_item.id) is True

    def test_upsert_updates_existing(
        self, db: merhist.database.Database, sample_sold_item: merhist.item.SoldItem
    ):
        """既存アイテムの更新"""
        db.upsert_sold_item(sample_sold_item)

        # 値を変更して更新
        updated_item = merhist.item.SoldItem(
    ...
    def test_exists_sold_item_false(self, db: merhist.database.Database):
        """存在しないアイテムは False"""
        assert db.exists_sold_item("nonexistent") is False

    def test_get_sold_count_empty(self, db: merhist.database.Database):
        """空のときは 0"""
        assert db.get_sold_count() == 0

    def test_get_sold_count(self, db: merhist.database.Database, sample_sold_item: merhist.item.SoldItem):
        """アイテム数を正しく返す"""
        db.upsert_sold_item(sample_sold_item)
        assert db.get_sold_count() == 1

        # 別のアイテムを追加
        item2 = merhist.item.SoldItem(id="m00000000002", name="商品2", price=500)
        db.upsert_sold_item(item2)
        assert db.get_sold_count() == 2

    def test_get_sold_item_list_empty(self, db: merhist.database.Database):
        """空のリストを返す"""
        items = db.get_sold_item_list()
        assert items == []

    def test_get_sold_item_list_ordered(self, db: merhist.database.Database):
    ...
    def test_sold_item_all_fields_preserved(
        self, db: merhist.database.Database, sample_sold_item: merhist.item.SoldItem
    ):
        """全フィールドが保存・復元される"""
        db.upsert_sold_item(sample_sold_item)
...
class TestSoldItemCategory:
    ...
    def test_category_json_serialization(self, db: merhist.database.Database):
    ...
    def test_empty_category(self, db: merhist.database.Database):
        """空のカテゴリは空リストに復元される"""
        item = merhist.item.SoldItem(id="m001", name="商品", category=[])
        db.upsert_sold_item(item)

        items = db.get_sold_item_list()
        assert items[0].category == []

    def test_category_with_unicode(self, db: merhist.database.Database):
        """日本語を含むカテゴリ"""
        item = merhist.item.SoldItem(
            id="m001",
            name="商品",
...
class TestSoldItemNullHandling:
    ...
    def test_null_fields_have_defaults(self, db: merhist.database.Database):
        """NULL フィールドはデフォルト値に変換される"""
        item = merhist.item.SoldItem(id="m001", name="最小限の商品")
        db.upsert_sold_item(item)
    ...
    def test_error_field_preserved(self, db: merhist.database.Database):
        """error フィールドが保存される"""
        item = merhist.item.SoldItem(id="m001", name="エラー商品", error="ページ読み込み失敗")
        db.upsert_sold_item(item)
...
class TestBoughtItemCRUD:
    ...
    def test_upsert_new_item(
        self, db: merhist.database.Database, sample_bought_item: merhist.item.BoughtItem
    ):
        """新規アイテムの挿入"""
    ...
    def test_upsert_updates_existing(
        ...
        updated_item = merhist.item.BoughtItem(
    ...
    def test_exists_bought_item_false(self, db: merhist.database.Database):
        """存在しないアイテムは False"""
        assert db.exists_bought_item("nonexistent") is False

    def test_get_bought_count_empty(self, db: merhist.database.Database):
        """空のときは 0"""
        assert db.get_bought_count() == 0

    def test_get_bought_count(
        self, db: merhist.database.Database, sample_bought_item: merhist.item.BoughtItem
    ):
        """アイテム数を正しく返す"""
        db.upsert_bought_item(sample_bought_item)
    ...
    def test_get_bought_item_list_ordered(self, db: merhist.database.Database):
...
class TestBoughtItemNullPrice:
    ...
    def test_null_price_preserved(self, db: merhist.database.Database):
        """price=None がそのまま保存・復元される"""
        item = merhist.item.BoughtItem(id="m001", name="価格なし商品", price=None)
...
class TestMetadata:
    ...
    def test_set_get_metadata(self, db: merhist.database.Database):
        """メタデータの保存・取得"""
        db.set_metadata("key1", "value1")
        assert db.get_metadata("key1") == "value1"

    def test_get_metadata_default(self, db: merhist.database.Database):
        """存在しないキーはデフォルト値"""
        assert db.get_metadata("nonexistent", "default") == "default"

    def test_get_metadata_empty_default(self, db: merhist.database.Database):
        """デフォルト値が空文字列"""
        assert db.get_metadata("nonexistent") == ""

    def test_update_metadata(self, db: merhist.database.Database):
        """メタデータの更新"""
        db.set_metadata("key1", "value1")
        db.set_metadata("key1", "value2")
        assert db.get_metadata("key1") == "value2"

    def test_set_get_metadata_int(self, db: merhist.database.Database):
        """整数メタデータの保存・取得"""
        db.set_metadata_int("count", 42)
        assert db.get_metadata_int("count") == 42

    def test_get_metadata_int_default(self, db: merhist.database.Database):
        """存在しない整数キーはデフォルト値"""
        assert db.get_metadata_int("nonexistent", 100) == 100

    def test_get_metadata_int_invalid_value(self, db: merhist.database.Database):
        """不正な値はデフォルト値に変換"""
        db.set_metadata("invalid", "not a number")
        assert db.get_metadata_int("invalid", 0) == 0

    def test_metadata_unicode(self, db: merhist.database.Database):
        """日本語メタデータ"""
...
class TestParseDatetime:
    ...
    def test_iso_format(self):
        """ISO 8601 形式のパース"""
        result = merhist.database.Database._parse_datetime("2025-01-15T10:30:00")
        assert result == datetime.datetime(2025, 1, 15, 10, 30, 0)

    def test_date_only(self):
        """日付のみのパース"""
        result = merhist.database.Database._parse_datetime("2025-01-15")
        assert result == datetime.datetime(2025, 1, 15, 0, 0, 0)

    def test_with_timezone(self):
        """タイムゾーン付きはタイムゾーンが削除される"""
        result = merhist.database.Database._parse_datetime("2025-01-15T10:30:00+09:00")
    ...
    def test_none_value(self):
        """None は None を返す"""
        result = merhist.database.Database._parse_datetime(None)
        assert result is None

    def test_empty_string(self):
        """空文字列は None を返す"""
        result = merhist.database.Database._parse_datetime("")
        assert result is None

    def test_invalid_format(self):
        """不正な形式は None を返す"""
        result = merhist.database.Database._parse_datetime("invalid")
        assert result is None
...
class TestPersistence:
    ...
    def test_data_persists_after_reopen(self, tmp_path: pathlib.Path):
        ...
        # データを挿入
        db1 = merhist.database.Database(db_path, SCHEMA_PATH)
        item = merhist.item.SoldItem(id="m001", name="永続化テスト", price=1000)
        db1.upsert_sold_item(item)
        db1.set_metadata("version", "1.0")
        db1.close()

        # 再オープン
        db2 = merhist.database.Database(db_path, SCHEMA_PATH)
...
class TestOpenDatabase:
    ...
    def test_open_database(self, tmp_path: pathlib.Path):
        ...
        assert isinstance(database, merhist.database.Database)
        database.close()
